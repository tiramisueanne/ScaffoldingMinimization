...
public:
  Indexer(int numVertices, set<int> internalNodes, set<pair<int, int>> allEdges)
  ...
  for (int i = 0; i < F.rows(); i++) {
    ...
    for (int j = 0; j < 3; j++) {
      ...
      for (int k = 0; k < 2; k++) {
        ...
        zDiff[indr.indexVert(currIndex)][indr.indexEdge(currIndex, other1)] =
            zDiff1;
#ifdef DEBUG
        cout << "We inserted" << zDiff1 << " into " << indr.indexVert(currIndex)
