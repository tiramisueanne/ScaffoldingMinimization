...
MatrixXd getNewPrimal(MatrixXd &reciprocalDual, MatrixXd &V, MatrixXi &F) {
    ...
    while (seenFaces.size() != F.rows()) {
        ...
        toProcess.pop();
        // for every one to nextFace, we check whether or not we have computed
        // the
        // gradient
        ...
        for (int i = 0; i < TT.row(currFace).size(); i++) {
            ...
            if (seenFaces.find(nextFace) == seenFaces.end() && nextFace != -1) {
                gradients.row(nextFace) = getNextGradient(
                    gradients.row(currFace), reciprocalDual.row(currFace),
                    reciprocalDual.row(nextFace));
                ...
                // (yes, one vertex from the previous triangle will be
                // overwritten)
                // (but this is okay due to the gradients giving the same result
                // at
                // shared points)
                for (int i = 0; i < 3; i++) {
                    if (i != indexMatching) {
                        newVerts.row(F(nextFace, i)) = applyGradient(
                            gradients.row(nextFace),
                            newVerts.row(F(nextFace, indexMatching)),
                            V.row(F(nextFace, i)));
                    }
                }
