...
MatrixXd QuadraticSolver::getNewDual() {
    // Adjacency matrix
        ...
        if (filledFaces.find(curFace) != filledFaces.end()) {
            ...
            for (int edgeInd = 0; edgeInd < 3; edgeInd++) {
                ...
                RowVector3d leftEdge =
                    edge * turnLeft *
                    weightMap.at(pair<int, int>(vert, nextVert));
#ifdef DEBUG
...
// Dual checking
bool QuadraticSolver::checkDual(MatrixXd &dualVerts) {
        ...
        for (int j = 0; j < 3; j++) {
            ...
            if (TT(i, j) != -1) {
                ...
                RowVector3d correctDualLength =
                    edgeDiff * weightMap.at(pair<int, int>({oneEnd, otherEnd}));
